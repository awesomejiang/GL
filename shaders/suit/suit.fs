...
void main(){
   //lights
   vec3 lightRes;
   lightRes += computeDirLight(dirLight);

   for(int i=0; i<4; ++i){
      lightRes += computePointLight(pointLight[i]);
   }

   lightRes += computeSpotLight(spotLight);

   //reflection
   vec3 I = normalize(objPos - viewPos);
   vec3 R = reflect(I, normalize(norm));
   vec4 reflectRes = vec4(texture(skybox, R).rgb, 1.0);

   FragColor = vec4(lightRes, 1.0) * reflectRes * 4;
   //FragColor = reflectRes;
